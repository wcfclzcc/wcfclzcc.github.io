    ...
    function Lchart(ctx_id,data)//折线图
    {
        ...
        this.data=data;
        var display_data_change=JSON.parse(JSON.stringify(data.display_data));
        ...
        var detial=new Draw_detial(ctx_id,height,width,data.display_data,mouse);

        var draw_all=new Draw_all(height,width);

        var zoom=new Mouse_zoom(ctx_id,height,width,data.display_data,mouse);
        ...
        function init() {//初始化设置
            ...
            that.set.zoom_set=setobj.zoom_set(2,3);
            let y_pos=[1];
            let x_pos=[1];
            let x_color=["#222"];
            let y_color=["#222"];
            ...
            let y_ms=[10];
            let x_max_index=that.data.date.length-1;
            let x_min_index=0;
            let y_max=[30];
            let y_min=[-30];
